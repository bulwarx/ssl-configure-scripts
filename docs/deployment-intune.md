...
Platform scripts run in the **SYSTEM** context by default (with an option to run in the logged-on user's context) — the same User-vs-System reasoning from the Win32 app section above applies; prefer running in the user's context if your Intune version offers that toggle for platform scripts.
