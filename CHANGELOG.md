...
- **README: documented packaging the cert bundle with an Intune Win32 app** — Intune extracts Win32 app content into a temporary staging folder that's deleted after install, so pointing `-CertBundle` at the package's own content path breaks shortly after deployment. Added a wrapper-script pattern that copies the bundle to a stable `%ProgramData%` location first, plus using that stable path as the Win32 app's detection rule, and a detection+remediation script pair for Intune Proactive Remediations that only attempts remediation when the bundle is actually still present.
