...
# an interactive login shell (e.g. Homebrew's /opt/homebrew/bin — where npm,
# pnpm, az, etc. actually live — is only added to PATH by .zprofile/.zshrc,
# which non-interactive/non-login shells don't source).
for extra_path in /opt/homebrew/bin /opt/homebrew/sbin /usr/local/bin /usr/local/sbin \
                  "$HOME/.cargo/bin" "$HOME/.local/bin" "$HOME/Library/pnpm"; do
